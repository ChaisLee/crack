...
ret1, th = cv2.threshold(out, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)  # Otsu 滤波
...
for istat in stats:
    ...
    i=i+1
cra = im.copy()
cv2.namedWindow('last vision',0)
cv2.imshow('last vision',im) 
cv2.waitKey(0)
